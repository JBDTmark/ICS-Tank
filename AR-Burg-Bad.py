...
def bic_criterion(variances, N):
    ...
    for p in range(1, len(variances)):
        bic_values[p] = N * np.log(variances[p]) + p * np.log(N)
        print(f"Order {p}: BIC = {bic_values[p]}")
    best_order = np.argmin(bic_values[1:]) + 1
...
def main():
    ...
    for i in range(len(predictions)):
        if i >= window_size:
            local_residual_mean = residuals[i-window_size:i].mean()
        else:
            local_residual_mean = residuals[:i+1].mean()
        predictions[i] += local_residual_mean
